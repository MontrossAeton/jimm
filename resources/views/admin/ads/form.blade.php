...
{{-- <div class="form-group">
    ...
    <select type="text" class="form-control{{ $errors->has('status') ? ' is-invalid' : '' }}" value="{{ isset($ad) ? $ad->status : old('status') }}" name="status" id="status">
        <option {{isset($ad) && $ad->status == "Pending" ? 'selected' : ''}} value="Pending">Pending</option>
        <option {{isset($ad) && $ad->status == "Approved" ? 'selected' : ''}} value="Approved">Approved</option>
    </select>
...
<div class="row pt-4">
    <div class="col ad-attachment-container {{ isset($ad) ? ($ad->attachment ? "" : "d-none") : "d-none" }}" id="{{ isset($ad) ? "ad_attachment_container_" . $ad->id : "ad_attachment_container" }}">
        <div class="btn-group-vertical" role="group" aria-label="Basic example">
            ...
            <button
                type="button"
                class="remove-ad-attachment-admin btn btn-outline-danger"
